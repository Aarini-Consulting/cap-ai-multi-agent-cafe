...
// Tools use internal CDS APIs (srv.send) instead of HTTP fetch,
// so user context and authorization propagate automatically.
let _orchestrator = null;
async function getOrchestrator(srv) {
  if (!_orchestrator) {
    const { createCafeOrchestrator } = await import('../agent/src/orchestrator.mjs');
    _orchestrator = createCafeOrchestrator(srv);
  }
...
module.exports = cds.service.impl(async function () {
  ...
  this.on('invokeAgent', async (req) => {
    ...
    try {
      const orchestrator = await getOrchestrator(this);
  ...
  this.on('getRecommendation', async (req) => {
    ...
    try {
      ...
      const text = `Based on your preference for "${preferences}" with a budget of EUR${budget}, I'd recommend the ${matching[0]?.name || "Chef's Special"} (EUR${matching[0]?.price || '?'}). ${matching.length > 1 ? `You could also try the ${matching[1]?.name} (EUR${matching[1]?.price}).` : ''} We have ${matching[0]?.stockQuantity || 0} in stock, so plenty available!`;
