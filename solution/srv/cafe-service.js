...
// Calls specialist agents via A2A protocol (HTTP) instead of in-process.
let _orchestrator = null;
async function getOrchestrator(req) {
  if (!_orchestrator) {
    const { createCafeOrchestrator } = await import('../agents/orchestrator.mjs');
    // Pass auth token so the orchestrator can forward it to A2A agents
...
module.exports = cds.service.impl(async function () {
  ...
  this.on('getItemDemand', async (req) => {
    ...
    // Check existing restock requests for this item
  ...
  this.on('getRestockDetails', async (req) => {
    ...
    // Also get demand data for this item
