...
interface ChatPanelProps {
  ...
  onFilter?: (filter: { view?: string; stockFilter?: string }) => void;
}

const QUICK_ACTIONS = [
  { label: 'Low stock items', message: 'Show me all items that are low on stock or out of stock' },
  { label: 'Pending restocks', message: 'What restock requests are currently pending?' },
  { label: 'Fulfill all pending', message: 'Can you fulfill all pending restock requests?' },
  { label: 'Stock forecast', message: 'Give me a stock forecast — which items will run out soon based on current levels?' },
];
...
export default function ChatPanel({ messages, isLoading, onSend, onFilter }: ChatPanelProps) {
  ...
  useEffect(() => {
    if (!hasSentInitial.current) {
      ...
      onSend('Give me a summary of current stock levels and any pending restock requests');
    }
  }, [onSend]);
    ...
    <div className="chat-panel">
      ...
      <div className="chat-input-bar">
        <input
          ...
          placeholder="Ask about stock levels, restocks, forecasts..."
          value={input}
