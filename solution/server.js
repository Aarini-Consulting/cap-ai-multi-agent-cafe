/**
 * Custom CDS server with audit logging and observability.
 *
 * Audit logging  — compliance: "who did what to which data?"
 *   → @cap-js/audit-logging → SAP Audit Log Viewer
 *
 * Observability  — operations: "what is happening and how fast?"
 *   → cds.log (structured) → SAP Cloud Logging / console
 *
 * @see https://cap.cloud.sap/docs/node.js/cds-serve#custom-server-js
 * @see https://cap.cloud.sap/docs/guides/data-privacy/audit-logging
 * @see https://cap.cloud.sap/docs/node.js/cds-log
 */
const cds = require('@sap/cds');

const LOG = cds.log('agent');       // structured logger for agent operations
const LOG_PERF = cds.log('agent.perf'); // performance/timing logger
...
// ═══════════════════════════════════════════════════════════════════
// AUDIT LOGGING — compliance trail for security and data access
// ═══════════════════════════════════════════════════════════════════

// ── Log 403 Forbidden responses as SecurityEvents ───────────────────
