...
async function odataGet(path: string): Promise<string> {
  try {
...
async function odataPost(path: string, body: Record<string, any>): Promise<string> {
  try {
    const res = await fetch(`${BASE_URL}${path}`, {
      method: "POST",
...
async function odataPostSSE(path: string, body: Record<string, any>): Promise<string> {
  try {
    const res = await fetch(`${BASE_URL}${path}`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    const chunks = text.split("\n")
      ...
      .filter(Boolean);
    return chunks.map(c => c.content || c.message || "").join("");
  } catch (e: any) {
...
export const getRecommendation = tool(
  async ({ preferences, budget }) => odataPostSSE("/api/cafe/getRecommendation", { preferences, budget }),
  { name: "getRecommendation", description: "Get AI meal recommendation (streamed)", schema: z.object({ preferences: z.string(), budget: z.number() }) }
);
...
export const generateComplaintResponse = tool(
  async ({ feedbackId }) => odataPostSSE("/api/cafe/generateComplaintResponse", { feedbackId }),
  { name: "generateComplaintResponse", description: "Generate an empathetic AI response to a complaint (streamed)", schema: z.object({ feedbackId: z.string() }) }
);
