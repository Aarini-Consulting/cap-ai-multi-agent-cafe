...
/**
 * Creates specialist runner functions bound to their tool groups.
 */
export function createSpecialists(cafeTools, kitchenTools, grievanceTools) {

  async function runCafeAssistant(context) {
    // logged by orchestrator
    return invokeSpecialist(
      cafeTools,
      `You are a friendly café assistant for the office cafeteria. Help customers browse the menu, place orders, and get recommendations.
...
- Be warm, helpful, and concise.`,
  ...
  async function runKitchenManager(context) {
    // logged by orchestrator
    return invokeSpecialist(
      kitchenTools,
      `You are the Kitchen Manager for the office café. You handle stock and inventory issues.
...
- Be efficient and practical. Report what alternatives are available and what you've restocked.`,
  ...
  async function runGrievanceManager(context) {
    // logged by orchestrator
    return invokeSpecialist(
      grievanceTools,
...
- Be empathetic but action-oriented.`,
  ...
  return { runCafeAssistant, runKitchenManager, runGrievanceManager };
}
