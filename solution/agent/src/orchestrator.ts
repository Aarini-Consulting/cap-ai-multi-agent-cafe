...
import { CafeOrchestratorState, type CafeOrchestratorStateType } from "./types.js";
import { runCafeAssistant, runKitchenManager, runGrievanceManager } from "./specialists.js";
...
async function orchestratorNode(
  state: CafeOrchestratorStateType
): Promise<Partial<CafeOrchestratorStateType>> {
...
`.trim();
  ...
  const response = await supervisorLLM.invoke([
    {
      role: "system",
      content: `You are the Café Orchestrator. You coordinate three specialist agents to serve customers.
...
async function cafeAssistantNode(
  state: CafeOrchestratorStateType
): Promise<Partial<CafeOrchestratorStateType>> {
  const context = [
...
async function kitchenManagerNode(
  state: CafeOrchestratorStateType
): Promise<Partial<CafeOrchestratorStateType>> {
  const context = [
...
async function grievanceManagerNode(
  state: CafeOrchestratorStateType
): Promise<Partial<CafeOrchestratorStateType>> {
  const context = [
...
function routeToAgent(state: CafeOrchestratorStateType): string {
